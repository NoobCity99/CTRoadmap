...
export function CanvasStylePreview({ selection }: CanvasStylePreviewProps) {
  ...
  return (
    ...
    >
      <CanvasBackgroundLayer background={background} previewMode />
      <div className="canvas-style-preview__overlay" style={overlayStyle} />
      {tile.tilePresentation === "cover-reveal" ? (
        <div className="cover-reveal-preview tile-node tile-node--service" data-tile-presentation={tile.tilePresentation}
          style={{ "--tile-accent": tile.accentColor } as CSSProperties}>
          <div className="tile-node__header">
            <span className="cover-reveal-art" aria-hidden="true" />
            <div className="tile-node__icon"><Box size={30} /></div>
            <div className="tile-node__title-wrap"><div className="tile-node__title">Example Service</div><div className="tile-node__type">Service</div></div>
          </div>
          <div className="tile-node__body">
            <div className="tile-node__fields"><div className="tile-node__field"><span>status</span><strong>Running</strong></div></div>
            <div className="tile-node__tags"><span>Hover to reveal</span></div>
          </div>
        </div>
      ) : <div
      ...
      >
        ...
        {theme.variant === "blueprint" && (
          <div className="tile-node__field canvas-style-preview__field" data-field-key="status">
            <span>status</span><strong>Running</strong>
          </div>
        )}
