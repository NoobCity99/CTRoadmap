...
import type { IconUploadResult, Tile, TileIconRef, UploadedIconAsset } from "../types/atlas";

export const UPLOADED_ICON_PREFIX = "uploaded:";
export const LUCIDE_ICON_PREFIX = "lucide:";

// A deleted image may already be decoded by the browser. Notify every glyph
// without rewriting the other tiles' saved references or touching Atlas state.
const deletedIcons = new Set<string>();
const deletionListeners = new Set<() => void>();
function subscribeToIconDeletion(listener: () => void) {
  deletionListeners.add(listener);
  return () => { deletionListeners.delete(listener); };
}
export function notifyIconDeleted(filename: string): void {
  deletedIcons.add(filename);
...
export function uploadedResultToIconRef(uploaded: IconUploadResult): UploadedIconAsset {
  return {
    kind: "uploaded",
...
export function TileIconGlyph({
  ...
  const uploadedUrl = iconRef?.kind === "uploaded" ? iconRef.url : null;
  const deleted = useSyncExternalStore(subscribeToIconDeletion,
    () => iconRef?.kind === "uploaded" && deletedIcons.has(iconRef.filename), () => false);
  const [failedUrl, setFailedUrl] = useState<string | null>(null);
  useEffect(() => setFailedUrl(null), [uploadedUrl]);

  if (iconRef?.kind === "uploaded" && !forceFallback && !deleted && failedUrl !== uploadedUrl) {
    return (
      <img
        alt={alt}
        height={size}
        width={size}
        onError={() => {
          setFailedUrl(iconRef.url);
          onUploadedError?.();
        }}
        src={iconRef.url}
        style={{ display: "block", flex: "0 0 auto", height: size, maxHeight: "100%", maxWidth: "100%", objectFit: "contain", width: size }}
      />
    );
  }
...
function isLucideIconName(name: string): name is keyof typeof LUCIDE_ICON_COMPONENTS {
  return Object.prototype.hasOwnProperty.call(LUCIDE_ICON_COMPONENTS, name);
}
